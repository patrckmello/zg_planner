 ...
 * @param {boolean} managerOnly - Se true, exige que o usuário seja gestor
...
function PrivateRoute({
  ...
  // Verifica permissões de gestor
  if (managerOnly && !user.is_manager && !user.is_admin) {
    console.warn(`Acesso negado à rota de Gestor: ${location.pathname}`);
    return (
      <Navigate
        to="/dashboard"
        state={{
          error: "Acesso negado. Permissões de Gestor necessárias.",
          from: location,
        }}
        replace
      />
    ...
    if (!hasRequiredRole) {
      ...
      return (
        <Navigate
          to="/dashboard"
          state={{
            error: `Acesso negado. Função necessária: ${requiredRoles.join(
              " ou "
            )}.`,
            from: location,
          }}
 ...
 * @param {boolean} props.managerOnly - Renderiza apenas para Gestors
