    ...
    if (isAdminRoute(config.url)) {
      ...
      if (userStr) {
        try {
          ...
          if (!user.is_admin) {
            console.warn('Tentativa de acesso a rota administrativa sem permissões');
            return Promise.reject(new Error('Acesso negado: permissões de administrador necessárias'));
          }
        } catch (error) {
...
api.interceptors.response.use(
  response => {
    ...
    if (isAdminRoute(response.config.url)) {
      console.log(`Requisição administrativa bem-sucedida: ${response.config.method?.toUpperCase()} ${response.config.url}`);
    }
    return response;
  },
  error => {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 403:
          ...
          if (isAdminRoute(url)) {
            console.error('Tentativa de acesso a rota administrativa sem permissões adequadas');
